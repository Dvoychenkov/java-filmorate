...
SELECT 1, 'G', 'General Audiences', 'Нет возрастных ограничений'
WHERE NOT EXISTS (SELECT 1 FROM mpa_ratings WHERE code = 'G');

INSERT INTO mpa_ratings (id, code, name, description)
SELECT 2, 'PG', 'Parental Guidance', 'Рекомендуется присутствие родителей'
WHERE NOT EXISTS (SELECT 1 FROM mpa_ratings WHERE code = 'PG');

INSERT INTO mpa_ratings (id, code, name, description)
SELECT 3, 'PG-13', 'Parents Strongly Cautioned', 'Не рекомендовано до 13'
WHERE NOT EXISTS (SELECT 1 FROM mpa_ratings WHERE code = 'PG-13');

INSERT INTO mpa_ratings (id, code, name, description)
SELECT 4, 'R', 'Restricted', 'До 17 лет с родителями'
WHERE NOT EXISTS (SELECT 1 FROM mpa_ratings WHERE code = 'R');

INSERT INTO mpa_ratings (id, code, name, description)
SELECT 5, 'NC-17', 'Adults Only', 'До 18 лет запрещено'
